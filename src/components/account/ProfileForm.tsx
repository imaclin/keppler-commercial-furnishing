...
export function ProfileForm({ name, email }: { name: string; email: string }) {
  ...
  return (
    <form action={async (fd) => { await updateProfileNameAction(fd); setSaved(true); }} className="max-w-md space-y-4">
      ...
      <div className="space-y-2"><Label htmlFor="name">Name</Label><Input id="name" name="name" value={value} onChange={(e) => { setValue(e.target.value); setSaved(false); }} required /></div>
