...
export async function sendQuoteAction(
  ...
  try {
    ...
    if (row) {
      const body = link
        ? `Your Keppler invoice is ready. View it and pay securely from your account: pay link ${link}`
        : 'Your Keppler invoice is ready. View it in your account.';
      await sendEmail(row.email, 'Your Keppler invoice is ready', body);
    }
  } catch {
